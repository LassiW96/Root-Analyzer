...
void Min_Sample_combined() {
    ...
    //Full range
    const Int_t upper_limit_full = 0;
    const Int_t lower_limit_full = 4096;
    ...
    for (Long64_t i = 0; i < chain1_nentries; i++) {
        chain1.GetEntry(i);
        chain1_min_ch2_sp = std::min_element(chain1_ch2 + upper_limit_full, chain1_ch2 + lower_limit_full) - chain1_ch2;
        chain1_min_ch2 = *std::min_element(chain1_ch2 + upper_limit_full, chain1_ch2 + lower_limit_full);
        chain1_first_below_indices.push_back(chain1_min_ch2_sp);
    }

    for (Long64_t i = 0; i < chain2_nentries; i++) {
    chain2.GetEntry(i);
    chain2_min_ch2_sp = std::min_element(chain2_ch2 + upper_limit_full, chain2_ch2 + lower_limit_full) - chain2_ch2;
    chain2_min_ch2 = *std::min_element(chain2_ch2 + upper_limit_full, chain2_ch2 + lower_limit_full);
    chain2_first_below_indices.push_back(chain2_min_ch2_sp);
}

    TH1I *hist_idx1 = new TH1I("Paddles-in", "Sample index of the ADC min; Sample idx; Count(log)", 100, upper_limit_full, lower_limit_full);

    for (auto idx : chain1_first_below_indices) {
        if (idx >= 0) hist_idx1->Fill(idx);
    }

    TH1I *hist_idx2 = new TH1I("Paddles-out", "Sample index variation of the ADC min; Sample idx; Count(log)", 100, upper_limit_full, lower_limit_full);
    ...
    latex->DrawLatexNDC(0.15, 0.65, "Combined Sample variation");
    ...
    c->SaveAs("MinSampleVariation_CombinedLogFullRange.png");
}
